...
public class CommandGamemodeAdventure implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
    	...
    	if(args.length == 0){
        	...
        	player.sendMessage(ChatColor.translateAlternateColorCodes('&', messages.getString("Messages.gamemode_adventure")));
        	return true;
      	} 
      	if(args.length == 1){
    	  Player playergma = Bukkit.getServer().getPlayer(args[0]);
        	if(playergma == null){
        	...
        	player.setGameMode(GameMode.ADVENTURE);
        	player.sendMessage(ChatColor.translateAlternateColorCodes('&', "&8[&cInsane&4Commands&8] &7Modo de Juego de " + args[0] + " &7establecido a &caventura&8."));
      	}else{
